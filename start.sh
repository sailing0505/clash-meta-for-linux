...
gen_config() {
	## Clash Meta 订阅地址检测及配置文件下载
	# 检查url是否有效
	echo -e '\n正在检测订阅地址...'
	Text1="ClashMeta订阅地址可访问！"
	Text2="ClashMeta订阅地址不可访问！"
	# #curl -o /dev/null -s -m 10 --connect-timeout 10 -w %{http_code} $URL | grep '[23][0-9][0-9]' &>/dev/null
	# curl -o /dev/null -L -k -sS --retry 5 -m 30 --connect-timeout 20 -w "%{http_code}" $URL | grep -E '^[23][0-9]{2}$' &>/dev/null
	# ReturnStatus=$?
	...
	# 尝试使用curl进行下载
	curl -L -k -sS --retry 5 -m 30 -o $Temp_Dir/clash.yaml $URL
	ReturnStatus=$?
	...
	## 判断订阅内容是否符合Clash Meta配置文件标准，尝试转换（当前不支持对 x86_64 以外的CPU架构服务器进行Clash Meta配置文件检测和转换，此功能将在后续添加）
	if [[ $CpuArch =~ "x86_64" || $CpuArch =~ "amd64" ]]; then
		echo -e '\n判断订阅内容是否符合Clash Meta配置文件标准:'
		bash $Server_Dir/scripts/clash_profile_conversion.sh
		sleep 3
	fi

	## Clash Meta 配置文件重新格式化及配置
	# 取出代理相关配置
	#sed -n '/^proxies:/,$p' $Temp_Dir/Clash Meta.yaml > $Temp_Dir/proxy.txt
	...
	# Configure Clash Meta Dashboard
	Work_Dir=$(
		cd $(dirname $0)
		pwd
	)
...
echo ''
echo -e "Clash Meta Dashboard 访问地址: http://<ip>:9090/ui"
